...
这是 `examples/agent-loop`（手写 ReAct 演示）的进阶版。`agent-loop` 用原始 fetch 手拼 XML 工具调用，帮你理解 ReAct 循环的底层机制。`mini-claude-code` 则换用 Vercel AI SDK，将工具注册、多步推理、消息拼接交给 SDK 处理，让你看到工业级写法是什么样的。
