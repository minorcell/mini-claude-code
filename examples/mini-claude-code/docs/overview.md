...
它的上一课是 `examples/agent-loop`——一个用原生 fetch + 手写 XML 解析展示 ReAct 基础原理的天气查询 Agent。`mini-claude-code` 在此基础上向前走一步：用真实的工具（文件读写、Shell 命令、网络请求）、工程级的框架选型，展示一个最接近 Claude Code 这类真实产品的最小内核。
